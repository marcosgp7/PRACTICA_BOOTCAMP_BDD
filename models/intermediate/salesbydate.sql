...
adjusted_times AS (
    ...
    JOIN {{ ref('dim_customers') }} c ON o.o_custkey = c.c_custkey
    JOIN {{ ref('dim_store') }} st ON UNIFORM(1, 48, RANDOM()) = st.st_store_id
    JOIN {{ ref('dim_reg_nation') }} n_store ON st.n_name = n_store.n_name
    JOIN {{ ref('dim_reg_nation') }} n_cliente ON c.n_name = n_cliente.n_name
)
SELECT 
    at.o_orderkey,
    at.o_orderdate,
